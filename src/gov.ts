...
/**
 * Submit a vote on a Onyx Governance proposal.
 *
 ...
 * 
 * (async function() {
 *   const castVoteTx = await onyx.castVote(12, 1);
...
export async function castVote(
  ...
  const errorPrefix = 'Onyx [castVote] | ';
  ...
  trxOptions._onyxProvider =  this._provider;
...
/**
 * Submit a vote on a Onyx Governance proposal using an EIP-712 signature.
 *
 * @param {string} proposalId The ID of the proposal to vote on. This is an
 *     auto-incrementing integer in the Governor contract.
 * @param {number} support A number value of 0, 1, or 2 for the proposal vote.
 ...
 * 
 * (async function() {
 *   const castVoteTx = await onyx.castVoteBySig(
 *     12,
 *     1,
 *     {
 *       v: '0x1b',
...
export async function castVoteBySig(
  ...
  const errorPrefix = 'Onyx [castVoteBySig] | ';
...
/**
 * Create a vote signature for a Onyx Governance proposal using EIP-712.
 ...
 * (async () => {
 *
 *   const voteForSignature = await onyx.createVoteSignature(20, 1);
 *   console.log('voteForSignature', voteForSignature);
 *
 *   const voteAgainstSignature = await onyx.createVoteSignature(20, 0);
...
/**
 * Submit a Onyx Governance proposal vote with a reason.
 *
 * @param {string} proposalId The ID of the proposal to vote on. This is an
 *     auto-incrementing integer in the Governor contract.
 * @param {number} support A number value of 0, 1, or 2 for the proposal vote.
 ...
 * 
 * (async function() {
 *   const castVoteTx = await onyx.castVoteWithReason(12, 1, 'I vote YES because...');
 *   console.log('Ethers.js transaction object', castVoteTx);
 * })().catch(console.error);
 * ```
 */
export async function castVoteWithReason(
  ...
  const errorPrefix = 'Onyx [castVoteWithReason] | ';
