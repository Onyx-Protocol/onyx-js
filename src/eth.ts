...
function _ethJsonRpc(
  ...
  return new Promise<any>((resolve, reject) => {
    const provider = options._onyxProvider || _createProvider(options);
 ...
 * @example
 * ```
 * const oEthAddress = Onyx.util.getAddress(Onyx.oETH);
 * 
 * (async function() {
 * 
 *   const srpb = await Onyx.eth.read(
 *     oEthAddress,
 ...
 *   );
 * 
 *   console.log('oETH market supply rate per block:', srpb.toString());
 * 
 * })().catch(console.error);
 * ```
 */
...
/**
 * This helps the Onyx.js constructor discover which Ethereum network the
 ...
 * (async function () {
 * 
 *   balance = await Onyx.eth.getBalance(myAddress, provider);
