...
/**
 * Get the balance of XCN tokens held by an address.
 *
 * @param {string} _address The address in which to find the XCN balance.
 * @param {Provider | string} [_provider] An Ethers.js provider or valid network
 *     name string.
 *
 * @returns {string} Returns a string of the numeric balance of XCN. The value
...
export async function getXcnBalance(
  ...
  const xcnAddress = address[net.name].XCN;
  const parameters = [ _address ];
  const trxOptions: CallOptions = {
    _onyxProvider: provider,
    abi: abi.XCN,
  };

  const result = await eth.read(xcnAddress, 'balanceOf', parameters, trxOptions);
  return result.toString();
}

/**
 * Get the amount of XCN tokens accrued but not yet claimed by an address.
 *
 * @param {string} _address The address in which to find the XCN accrued.
 * @param {Provider | string} [_provider] An Ethers.js provider or valid network
 *     name string.
 *
 * @returns {string} Returns a string of the numeric accruement of XCN. The
...
export async function getXcnAccrued(
  ...
  const lensAddress = address[net.name].OnyxLens;
  const xcnAddress = address[net.name].XCN;
  const comptrollerAddress = address[net.name].Comptroller;
  const parameters = [ xcnAddress, comptrollerAddress, _address ];
  const trxOptions: CallOptions = {
    _onyxProvider: provider,
    abi: abi.OnyxLens,
  };
...
/**
 * Create a transaction to claim accrued XCN tokens for the user.
 *
...
/**
 * Create a transaction to delegate Onyx Governance voting rights to an
 *     address.
 *
 ...
 * 
 * (async function() {
 *   const delegateTx = await onyx.delegate('0xa0df350d2637096571F7A701CBc1C5fdE30dF76A');
...
export async function delegate(
  ...
  const errorPrefix = 'Onyx [delegate] | ';
  ...
  const xcnAddress = address[this._network.name].XCN;
  const trxOptions: CallOptions = {
    ...options,
    _onyxProvider: this._provider,
    abi: abi.XCN,
  ...
  return eth.trx(xcnAddress, method, parameters, trxOptions);
}

/**
 * Delegate voting rights in Onyx Governance using an EIP-712 signature.
 *
...
export async function delegateBySig(
  ...
  const errorPrefix = 'Onyx [delegateBySig] | ';
...
/**
 * Create a delegate signature for Onyx Governance using EIP-712. The
 ...
 * (async () => {
 *
 *   const delegateSignature = await onyx.createDelegateSignature('0xa0df350d2637096571F7A701CBc1C5fdE30dF76A');
